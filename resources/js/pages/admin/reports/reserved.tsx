...
type AdminReservedReportPageProps = {
    reservations: PaginatedResponse<ReportReservation>;
    users: ReportFilterUser[];
    terrains: ReportFilterTerrain[];
    filters: ReservedReportFilters;
};

export default function AdminReservedReportPage({
    reservations,
    users,
    terrains,
    filters,
}: AdminReservedReportPageProps) {
    const { t } = useI18n();
    ...
    return (
        <AdminSectionLayout
            title={t('report_reserved_title')}
            description={t('report_reserved_description')}
        >
            <Head title={t('report_reserved_title')} />
            ...
            <Card>
                ...
                <CardContent>
                    <form className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" onSubmit={applyFilters}>
                        ...
                        <div className="space-y-2">
                            <Label>{t('report_period')}</Label>
                            <Select value={period} onValueChange={setPeriod}>
                                <SelectTrigger>
                                    <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="all">{t('report_period_all')}</SelectItem>
                                    <SelectItem value="upcoming">{t('report_period_upcoming')}</SelectItem>
                                    <SelectItem value="past">{t('report_period_past')}</SelectItem>
                                </SelectContent>
                            </Select>
                        </div>
            ...
            <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                    <CardTitle>{t('report_reserved_title')}</CardTitle>
                    <span className="text-sm text-muted-foreground">
                        {t('report_total')}: {reservations.meta?.total ?? reservations.data.length}
                    </span>
                </CardHeader>
                <CardContent className="space-y-4">
                    <ReportReservationsTable reservations={reservations.data} />
                    <ReportPagination
                        meta={reservations.meta}
                        routePath="/admin/reports/reserved"
                        query={{
