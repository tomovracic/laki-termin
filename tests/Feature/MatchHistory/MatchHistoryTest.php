...
test('admin can update casual match they are not part of', function () {
    $admin = User::factory()->create();
    assignLeagueAdminRole($admin);
    ...
    $this->actingAs($admin)->patchJson(route('played-matches.update', $playedMatch), [
        'set1_player_one_games' => 7,
        'set1_player_two_games' => 5,
        'set2_player_one_games' => 6,
        'set2_player_two_games' => 4,
...
test('admin can delete casual match they are not part of', function () {
    $admin = User::factory()->create();
    assignLeagueAdminRole($admin);
    ...
    $this->actingAs($admin)->deleteJson(route('played-matches.destroy', $playedMatch))
        ->assertOk()
        ->assertJsonPath('data.deleted', true);
...
test('match history exposes edit flags for admin on other users casual matches', function () {
    $admin = User::factory()->create();
    assignLeagueAdminRole($admin);
...
test('match history exposes edit flag for admin on league matches', function () {
    $admin = User::factory()->create();
    assignLeagueAdminRole($admin);
    ...
    $this->actingAs($admin)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('matches.0.can_edit', true)
            ->where('matches.0.can_delete', false)
            ->where('matches.0.league.sets_best_of', 3));
});

test('admin can update league match result from match history endpoint', function () {
    $admin = User::factory()->create();
    assignLeagueAdminRole($admin);
    ...
    $this->actingAs($admin)->patchJson(route('leagues.matches.result.update', [
        'league' => $league,
        'match' => $match,
    ]), [
        'set1_player_one_games' => 4,
        ...
        ->assertJsonPath('data.set1_player_one_games', 4)
        ->assertJsonPath('data.set1_player_two_games', 6);
